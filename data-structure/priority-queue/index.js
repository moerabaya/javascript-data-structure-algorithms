...
class PriorityQueue {
    ...
    enqueue(value, priority) {
        ...
        while (move) {
            ...
            if (parentIdx < 0 || !parent || parent.priority < node.priority)
                move = false;
            else {
    ...
    dequeue() {
        ...
        if (this.values.length > 0) {
            ...
            while (
                left &&
                right &&
                (end.priority > left.priority || end.priority > right.priority)
            ) {
                let swapIdx =
                    left.priority > right.priority ? rightIdx : leftIdx;
